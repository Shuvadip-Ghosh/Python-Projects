...
# Try with your own pictures and Do not use against any photos that you don't own or have authorization to test. 
...
while True:
	...
	try:
		...
		if conv_val == 1:
			...
			print("Please select the folder in which you want to have the file.")
			time.sleep(1)
			root = Tk()
			root.withdraw()
			folder_selected = filedialog.askdirectory()
			sys.stdout = open(f"{folder_selected}\\exif_data.txt", "w")
			break
		elif conv_val == 2:
...
print("Please select the image(s) you want to process....")
time.sleep(1.5)
root = Tk()
root.withdraw()
files = list(filedialog.askopenfilename(title='Select an Image',filetypes=(('images', ('*.tiff','*.jpg')),('JPEG images', '*.jpg'))))

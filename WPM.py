...
def start_screen(stdscr):
	...
	for x in uc:
		...
		r+=1
	stdscr.addstr(s+4,s,"Enter the letter you want to continue with:- ")
	rectangle(stdscr, s+3,s+44,s+5,s+48)
	win = curses.newwin(1,2,s+4,s+46)
	box = Textbox(win)
	stdscr.addstr(s+4,s+49,"Press CTRL-G to confirm")
	stdscr.refresh()
	box.edit()
	return str(box.gather()).strip()

def display_text(stdscr, target, current,r, wpm=0):
	stdscr.addstr(2,6,"Practising Letter:- ")
	stdscr.addstr(2,26,f" {r} ",curses.A_REVERSE)
	stdscr.addstr(3,6,target)
	stdscr.addstr(4, 6, f"WPM: {wpm}")
	...
	for i, char in enumerate(current):
		...
		stdscr.addstr(3, i+6, char, color)
...
def wpm_test(stdscr,letter):
	...
	while True:
		...
		stdscr.clear()
		display_text(stdscr, target_text, current_text,letter, wpm)
		stdscr.refresh()
...
def main(stdscr):
	...
	while True:
		try:
			s = start_screen(stdscr)
			wpm_test(stdscr,s.upper())
			stdscr.addstr(5,6,"You completed the text!\n      Press Esc to exit or\n      Press any key to continue...")
		except:
			stdscr.addstr(5,6,"An Error occurred!\n      Press Esc to exit or \n      Press any key to continue...")
		key = stdscr.getkey()
		if ord(key)==27:
			curses.endwin()
			break
